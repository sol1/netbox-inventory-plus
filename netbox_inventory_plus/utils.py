...
def get_plugin_setting(setting_name):
    return get_plugin_config('netbox_inventory_plus', setting_name)
...
def get_status_for(status):
    ...
    if status_name not in dict(AssetStatusChoices):
        raise ImproperlyConfigured(
            f'netbox_inventory_plus plugin configuration defines status {status_name}, but it is not defined in FIELD_CHOICES["netbox_inventory_plus.Asset.status"]'
        )
    return status_name
...
def get_all_statuses_for(status):
    ...
    if extra_statuses := status_names.difference(set(dict(AssetStatusChoices))):
        raise ImproperlyConfigured(
            f'netbox_inventory_plus plugin configuration defines statuses {extra_statuses}, but these are not defined in FIELD_CHOICES["netbox_inventory_plus.Asset.status"]'
        )
